...
class CornellMovieCorpusProcessor:
    ...
    def get_conversations(self):
        ...
        with open(self.movie_conversations, 'r', encoding='iso-8859-1') as f:
            for line in f:
                items = line.split(DELIM)
                conversation_ids_field = items[conversation_ids_index] #对话顺序
                conversation_ids = literal_eval(conversation_ids_field)  # 字符串转换成 list
...
def tensor_from_sentence(type, lang, sentence, max_length):
    indexes = indexes_from_sentence(lang, sentence) #获取 词汇id
...
def tensors_from_pair(input_lang, output_lang, pair, max_length_en, max_length_de):
    input_tensor = tensor_from_sentence("en", input_lang, pair[0], max_length_en) # word -> id, 补齐，转换成tesor
...
def load_data(max_length):
    pairs = []
    language = Lang("english")  # 词典

    processor = CornellMovieCorpusProcessor("Dataset/movie_lines.txt", "Dataset/movie_conversations.txt")
    print('Collection line-ids...')
    id2lines = processor.get_id2line()  # 返回所有对话的文本和文本id {id:text,id:text,}
    print('Collection conversations...')
    conversations = processor.get_conversations()   #返回所有的对话过程信息 [[id1,id2,id6],[id8,id4]]
    print('Preparing question/answer sets...')
    questions, answers = processor.get_question_answer_set(id2lines, conversations) #得到 问题和回答，问题和回答一一对应

    for question, answer in zip(questions, answers):
        pairs.append([question, answer])

    pairs = filter_pairs(pairs, max_length) #过滤操作，过滤掉长度大于max_length的问答
    for pair in pairs:
        language.add_sentence(pair[0])
        language.add_sentence(pair[1])  #预料填充词典
...
def data_generator(batch_size, max_length, device):
    total_data = []
    language, pairs = load_data(max_length) #返回词典 与 问答数据
    ...
    return language, total_data #返回词典和数据
