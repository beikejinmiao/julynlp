...
class EncoderRNN(nn.Module):
	def __init__(self, featureSize, hiddenSize, embedding, numLayers=1, dropout=0.1, bidirectional=True):
		...
		self.featureSize = featureSize # GRU输入大小 = embedding的维度大小
	...
	def forward(self, input, lengths, hidden):
		...
		if self.bidirectional:
			...
			#output: batchSize × seq_len × hiddenSize
		return output, hn
...
class DecoderRNN(nn.Module):
	...
	def forward(self, input, hidden):
		# input: batchSize × seq_len; hidden: numLayers × batchSize × hiddenSize
		input = self.embedding(input)  # => batchSize × seq_len × feaSize
		input = F.relu(input)
		output, hn = self.gru(input,
							  hidden)  # output: batchSize × seq_len × feaSize; hn: numLayers × batchSize × hiddenSize
...
class BahdanauAttentionDecoderRNN(nn.Module):
	...
	def forward(self, inputStep, hidden, encoderOutput):
		# inputStep: batchSize × 1; hidden: numLayers × batchSize × hiddenSize ；encoderOutput：batchSize × seq_len × hiddenSize;
		...
		# 1 × batchSize × hiddenSize --> seq_len × batchSize × hiddenSize -->  batchSize × seq_len × hiddenSize
		# --> batchSize × seq_len × hiddenSize* 2
		#[batchSize,seq_len,hiddenSize * 2] * [batchSize,hiddenSize * 2, 1] = [batchSize,seq_len ,1]
		# transpose--> [batchSize , 1 , seq_len]
		# attentionWeight: batchSize × 1 × seq_len  torch.cat 拼接

		# bmm批  矩阵乘操作 attentionWeight:batchSize × 1 × seq_len   encoderOutput: batchSize × seq_len × hiddenSize
		context = torch.bmm(attentionWeight, encoderOutput)  # context: batchSize × 1 × hiddenSize
		attentionCombine = self.attention_combine(
			torch.cat((inputStep, context), dim=2))
		#inputStep:batchSize × 1 × feaSize  context: batchSize × 1 × hiddenSize --> batchSize × 1 × hiddenSize+feaSize
		#batchSize × 1 × hiddenSize+feaSize * batchSize x featureSize + hiddenSize x featureSize --> batchSize × 1 × featureSize
		# attentionCombine: batchSize × 1 × feaSize
...
class Seq2Seq:
	...
	def train(self, batchSize, isDataEnhance=False, dataEnhanceRatio=0.2, epoch=100, stopRound=10, lr=0.001,
		...
		dataStream = self.dataClass.random_batch_data_stream(batchSize=batchSize, isDataEnhance=isDataEnhance,
															 dataEnhanceRatio=dataEnhanceRatio) #数据流 数据增强+seq长度补齐
		if self.dataClass.testSize > 0:
			testStrem = self.dataClass.random_batch_data_stream(batchSize=batchSize, type='test') #数据流 数据增强+seq长度补齐
...
class ChatBot:
	...
	def predictByGreedySearch(self, inputSeq, maxAnswerLength=32, showAttention=False, figsize=(12, 6)):
		inputSeq = filter_sent(inputSeq) # 过滤
		inputSeq = [w for w in jieba.lcut(inputSeq) if w in self.word2id.keys()] #分词

		X = seq2id(self.word2id, inputSeq) # 获取词id
		...
		while decoderInput.item() != eosToken and len(Y) < maxAnswerLength:
			...
			decoderInput = topi[:, :, 0]  #  batchSize × 1;
			attentionArrs.append(decoderAttentionWeight.data.cpu().numpy().reshape(1, XLens))
			Y.append(decoderInput.item())
		outputSeq = id2seq(self.id2word, Y) # 根据id获取对应的词 1 × leq_len;
		#decoderAttentionWeight:1 × 1 × seq_len
		#attentionArrs: 1 × seq_len × seq_len
	...
	def predictByBeamSearch(self, inputSeq, beamWidth=10, maxAnswerLength=32, alpha=0.7, isRandomChoose=False,
		...
		for i in range(1, maxAnswerLength):
			...
			if improve:
				...
				if allRandomChoose:
					topv, topi = self._random_pick_k_by_prob(decoderOutput, k=beamWidth) #随机选择 beamWidth 个
				else:
					topv, topi = decoderOutput.topk(beamWidth, dim=0)  #按概率大小选择 前beamWidth 个
			else:
				topv, topi = (torch.tensor(prob[localRestId], dtype=torch.float32, device=self.device).unsqueeze(
					2) + decoderOutput).view(-1, 1).topk(beamWidth, dim=0) #与前一个进行相加
			#unsqueeze：在第2维增加一维 prob：beamWidth x 1 x 1 ;decoderOutput: beamWidth × 1 × outputSize -->　（beamWidthｘoutputSize，１）
			#topv: beamWidth × 1; topi: beamWidth × 1

			# decoderInput: beamWidth × 1
			decoderInput = topi % outputSize #归一化 outputSize=单词数量
		...
		lens = [i.index(eosToken) if eosToken in i else maxAnswerLength for i in Y.tolist()] #计算长度
		ans = [''.join(id2seq(self.id2word, i[:l])) for i, l in zip(Y, lens)] #根据id获取词
...
def _calculate_loss(encoderRNN, decoderRNN, X, XLens, Y, YLens, teacherForcingRatio, device):
	...
	X, Y = torch.tensor(X, dtype=torch.long, device=device), torch.tensor(Y, dtype=torch.long, device=device) #转换成tensor
	...
	#encoderOutput: batchSize × seq_len × hiddenSize;
	#hidden: numLayers * d × batchSize × hiddenSize
	...
	for i in range(YSeqLen):
		...
		if random.random() < teacherForcingRatio:
			...
			topv, topi = decoderOutput.topk(1) #在最后一维 取最大值 topi：batchSize × 1 × 1
			decoderInput = topi[:, :, 0]  #  batchSize × 1;
		restId = (YLens > i + 1).view(-1) # [True,False,....]
		decoderInput = decoderInput[restId]
		hidden = hidden[:, restId, :]
		encoderOutput = encoderOutput[restId]
		Y = Y[restId]
